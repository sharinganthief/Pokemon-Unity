...
public class BattleHandler : MonoBehaviour {
			...
			if(selectedPokemon == null){
				...
				if(i == 0){
					if(i == pokePartyPosition){
						if(selectedPokemon.getStatus() != Pokemon.Status.FAINTED){
	...
	private void updateSelectedBagCategory(int newPosition){
		...
		for(int i = 0; i < 6; i++){
			if(i != bagCategoryPosition){ //deselect
				if(i == 4){
	...
	private void updateSelectedPokemonSlot(int newPosition, bool backSelectable){
		...
		if(newPosition < 6){
			if(SaveData.currentSave.PC.boxes[0][newPosition] == null){
				...
				bool spaceFound = false;
				if(newPosition < pokePartyPosition){ //keep going back 1 until avaiable position found
					while(!spaceFound){
						checkPosition -= 1;
						if(checkPosition < 0){
				...
				else{ //keep going forward 1
					while(!spaceFound){
						checkPosition += 1;
						if(checkPosition > 5){
							if(backSelectable){
								newPosition = 6;} //set position to Back button
							else{
								newPosition = 5;
		...
		for(int i = 0; i < 7; i++){
			if(i != pokePartyPosition){ //unhighlight
				if(i == 0){
	...
	private float calculateModifiedDamage(int attackerPosition, int targetPosition, MoveData move, float baseDamage, bool applyCritical){
		...
		if(move.getCategory() == MoveData.Category.PHYSICAL){
			...
			if(!applyCritical){ //exclude defensive buffs in a critical hit
	...
	private int getHighestSpeedIndex(){
		...
		for(int i = 0; i < 6; i++){
			if(!pokemonHasMoved[i]){
				...
				else if(command[i] == CommandType.Switch){ //6 priority for regular swapping out.
					calculatedPokemonPriority = 6;}
		...
		if(chosenIndex == 1){
			...
			while(learning){
				...
				if(selectedPokemon.getMoveCount() == 4){
					...
					if(chosenIndex == 0){ //NOT ELSE because this may need to run after (chosenIndex == 1) runs
	...
	private IEnumerator ModifyStat(int targetPosition, int statIndex, float param, bool animate){
		...
		bool canModify = true;
		if(pokemonStatsMod[statIndex][targetPosition] >= 6 && parameter > 0){ //can't go higher
			...
			canModify = false;}
		else if(pokemonStatsMod[statIndex][targetPosition] <= -6 && parameter < 0){ //can't go lower
	...
	//		Debug.Log(pokemon[3].getName()+": HP: "+pokemon[3].getHP()+"ATK: "+pokemon[3].getATK()+"DEF: "+pokemon[3].getDEF()+"SPA: "+pokemon[3].getSPA()+"SPD: "+pokemon[3].getSPD()+"SPE:"+pokemon[3].getSPE());
			...
			while(runState){
				...
				if(Input.GetAxisRaw("Horizontal") < 0){
					...
					if(taskPosition > 2){
						if(taskPosition == 4){ //if run selected
					...
					if(taskPosition < 3){
						if(taskPosition == 1){ //if fight selected
							updateSelectedTask(taskPosition + 3);
							SfxHandler.Play(scrollClip);
							yield return new WaitForSeconds(0.2f);
						}
						...
						while(currentTask == 1){
							if(Input.GetAxisRaw("Horizontal") < 0){
								if(movePosition == 1){
									...
									if(movePosition == 5){
										if(pokemonMoveset[currentPokemon][2] != null){ //check destination has a move there
								...
								if(movePosition < 5 && movePosition != 2){
									if(movePosition == 1){
										if(pokemonMoveset[currentPokemon][1] != null){ //check destination has a move there
											updateSelectedMove(movePosition + 1);
											SfxHandler.Play(scrollClip);
											yield return new WaitForSeconds(0.2f);}
									}
										...
										if(pokemonMoveset[currentPokemon][3] != null){ //check destination has a move there
											updateSelectedMove(movePosition + 1);
											SfxHandler.Play(scrollClip);
											yield return new WaitForSeconds(0.2f);}
										else if(pokemonMoveset[currentPokemon][1] != null){ //check there is a move 2
								...
								if(movePosition == 3){
									if(canMegaEvolve){
										...
										SfxHandler.Play(scrollClip);
										yield return new WaitForSeconds(0.2f);}
									else{ //otherwise, go down to return
										updateSelectedMove(1);
										SfxHandler.Play(scrollClip);
										yield return new WaitForSeconds(0.2f);}
								}
								...
								if(movePosition < 3){
									if(movePosition == 1){
										...
										if(pokemonMoveset[currentPokemon][3] != null){ //check destination has a move there
											...
											SfxHandler.Play(scrollClip);
											yield return new WaitForSeconds(0.2f);}
										else if(pokemonMoveset[currentPokemon][2] != null){ //check if there is a move 3
								...
								if(movePosition == 0){ //if mega evolution selected (mega evolution not yet implemented)
								}
								else if(movePosition == 3){ //if back selected
									SfxHandler.Play(selectClip);
									updateCurrentTask(0);}
								else{ //if a move is selected
								...
								if(!runState){ //if a move was chosen.
									SfxHandler.Play(selectClip);
					...
					//// OPEN BAG INTERFACE ////
						...
						while(currentTask == 2){
							...
							if(Input.GetAxisRaw("Vertical") < 0){
								...
								if(bagCategoryPosition < 4){ //Item Category
									updateCurrentTask(4);
									SfxHandler.Play(selectClip);
									...
									while(currentTask == 4){
										if(Input.GetAxisRaw("Vertical") < 0){
											...
											if(itemListPosition == 8){
												...
												if(!runState){ //if an item was chosen.
													SfxHandler.Play(selectClip);
								...
								else if(bagCategoryPosition == 4){ //Back
									updateCurrentTask(0);
									SfxHandler.Play(selectClip);
									yield return new WaitForSeconds(0.2f);
								}
								else if(bagCategoryPosition == 5){ //Item used last
									SfxHandler.Play(selectClip);
									yield return new WaitForSeconds(0.2f);
								}
							}
								...
								if(pokePartyPosition == 6){ //Back
									...
									while(currentTask == 5){
										...
										if(Input.GetAxisRaw("Vertical") < 0){
											...
											if(summaryPosition == 0){
											...
											else if(summaryPosition == 1){//check moves
												updateCurrentTask(6);
												SfxHandler.Play(selectClip);
												yield return new WaitForSeconds(0.2f);
												...
												while(currentTask == 6){
													...
													if(Input.GetAxisRaw("Vertical") < 0){
														...
														else if(movesPosition == 5){//check summary
															updateCurrentTask(5);
															SfxHandler.Play(selectClip);
															yield return new WaitForSeconds(0.2f);
														}
														else if(movesPosition == 6){//back
															updateCurrentTask(3);
															SfxHandler.Play(selectClip);
															yield return new WaitForSeconds(0.2f);
														}
											...
											else if(summaryPosition == 2){//back
												updateCurrentTask(3);
												SfxHandler.Play(selectClip);
												yield return new WaitForSeconds(0.2f);
											}
			...
			for(int i = 0; i < 6; i++){
				if(running){ //running may be set to false by a flee command
					int movingPokemon = getHighestSpeedIndex();
					if(pokemon[movingPokemon] != null){

						if(command[movingPokemon] == CommandType.Flee){					//RUN
							if(movingPokemon < 3){	//player attemps escape
						...
						else if(command[movingPokemon] == CommandType.Item){	//ITEM
								...
								if(commandItem[movingPokemon].getItemEffect() == ItemData.ItemEffect.BALL){
									...
									//calculate catch chance
										...
										if(shakes == 4){
											...
											if(chosenIndex == 1){ //give nickname
												SfxHandler.Play(selectClip);
									...
									if(commandTarget[movingPokemon] < 3){ //if target is player
						...
						else if(command[movingPokemon] == CommandType.Move){	//MOVE
							//debug autoselect targetIndex (target selection not yet implemented)
							int targetIndex = 3;
							...
							if(pokemon[movingPokemon].getStatus() != Pokemon.Status.FAINTED){
								...
								if(canMove){ //use the move
									...
									//adjust for accuracy
									if(accuracy != 0 && Random.value > accuracy){ //if missed, provide missed feedback
								...
								if(canMove){ //if didn't miss
									...
									if(commandMove[movingPokemon].hasMoveEffect(MoveData.Effect.Heal)){
										...
										if(commandMove[movingPokemon].getCategory() == MoveData.Category.PHYSICAL){
											if(applyCritical){		//if a critical lands
												if(pokemonStatsMod[0][movingPokemon] > 0){ //only apply ATKmod if positive
													damageToDeal *= calculateStatModifier(pokemonStatsMod[0][movingPokemon]);}
												if(pokemonStatsMod[1][targetIndex] < 0){ //only apply DEFmod if negative
													damageToDeal *= calculateStatModifier(pokemonStatsMod[1][targetIndex]);}
											}
											else{	//apply ATK and DEF mods normally (also half damage if burned)
												damageToDeal *= calculateStatModifier(pokemonStatsMod[0][movingPokemon]);
												damageToDeal /= calculateStatModifier(pokemonStatsMod[1][targetIndex]);
											...
											if(applyCritical){	//same as above, only using the Special varients
									...
									if(targetIndex == 0){ //if player pokemon 0 (only stats bar to display HP text)
						...
						else if(command[movingPokemon] == CommandType.Switch){	//switch pokemon
					...
					else{ //count pokemon as moved as pokemon does not exist.
						pokemonHasMoved[movingPokemon] = true;
					}
				}
			}
			...
			if(running){ //running may be set to false by a successful flee
				...
				for(int i = 0; i < 6; i++){
					if(pokemon[i] != null){
						if(pokemon[i].getStatus() == Pokemon.Status.BURNED || pokemon[i].getStatus() == Pokemon.Status.POISONED){
							...
							if(i == 0){ //if player pokemon 0 (only stats bar to display HP text)
					...
					for(int i = 0; i < pokemonPerSide; i++){
						...
						if(pokemon[i+3] == null){
							...
							for(int i2 = 0; i2 < opponentParty.Length; i2++){
								if(opponentParty[i2].getStatus() != Pokemon.Status.FAINTED){
									//check that pokemon is not on the field
									bool notOnField = true;
		...
		//if defeated
		if(victor == 1){ //empty the paused clip, as the paused audio won't be resumed upon respawning
			BgmHandler.main.ResumeMain(1.4f, null, 0);
		}
		else{ //if not defeated, the scene won't have faded out already
			StartCoroutine(ScreenFade.main.Fade(false, 1f));
